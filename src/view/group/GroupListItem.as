package view.group
{
	 ...
	 * @author RexJiang
	 */
	public class GroupListItem extends BaseComponent implements IListItem 
	{
		...
		private var _groupMCList:Array;
		...
		private function init():void
		{
			...
			for (var i:int = 0; i < count; i++)
			{
				...
				_groupMCList.push(item.btn_select);
				item.btn_select.buttonMode = true;
				item.btn_select.index = i;
				item.btn_select.addEventListener(MouseEvent.CLICK, onBtnSelectGroupClick);
				_mcItemList.push(item);
			}
		}
		
		private function onBtnSelectGroupClick(e:MouseEvent):void
		{
			...
			{
				item.gotoAndStop(2);
				_data[index].preSelectGroup = true;
			}
			else
			{
				item.gotoAndStop(1);
				_data[index].preSelectGroup = false;
			}
		...
		override public function set selected(value:Boolean):void 
		{
			...
			for (var i:int = 0; i < count; i++)
			{
				if (i < _data.length)
				{
					if (value )
					{
						_groupMCList[i].gotoAndStop(2);
						_data[i].preSelectGroup = true;
					}
					else
					{
						_groupMCList[i].gotoAndStop(1);
						_data[i].preSelectGroup = true;
					}
				}
			}
